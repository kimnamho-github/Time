 ...
 */
@WebServlet("/adminTodayInsert.to")
public class TodayInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			LocalDate localDate = LocalDate.parse(multiRequest.getParameter("boardDate"));
			...
			if(result > 0) {
				...
				response.sendRedirect(request.getContextPath()+"/adminBoardView.bo?cPage=1&bt=B2");
			}else {
				// 실패시 에러페이지
