...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.ArrayList, com.user.board.open.model.dao.BoardDao
    							,com.user.board.open.model.vo.Board
...
<body>
	...
	<div id="btn-outer">
		<div class="btn-r" style="vertical-align: middle">
			<!-- 버튼에는 a태그와 같이 경로이동 속성인 href 속성이 없기 때문에 이벤트로 추가해서 작성해야한다. -->
			<!-- <button onclick="location.href='/jsp/enrollForm.no'">글작성</button> -->
			<a href="<%=contextPath%>/enrollForm.op"><img
				src="resources/image/common/chat.png" id="r-logo1"><span>자유게시판
					작성</span></a>
	...
	<div class="outer1">
		...
		<div id="list1">
			<h5 id="nt">
				<img src="resources/image/common/chat.png" id="r-logo">Open
				Board
			</h5>
		</div>
			...
			<table id="list-area" align="center" class="table table-hover">
				...
				<tbody>
					...
					<tr id="t2">
						...
						<td width="500"><img
							src="resources/image/common/megaphone.png" id="r-logo">자유게시판
							작성시 주의사항 안내</td>
						<td width="150" align="center">admin</td>
						<td width="150" align="center">2022-11-15</td>
					...
					<tr class="table-light" id="l1">
						...
						<td id="te"><%=b.getBoardWriter()%></td>
						<td id="te" align="center"><%=b.getBoardTitle()%></td>
		...
		<script>
        	$(function(){
        		$("#list-area>tbody>tr").click(function(){
        		 console.log(this);
        			location.href="<%=request.getContextPath()%>/detail.op?ono="+ $(this).children().eq(0).text();
				})
			});
		</script>
		...
		<div align="center" id="ri" style="text-align: center;">
			...
			<button class="paging-b"
				onclick="location.href='<%=request.getContextPath()%>/list.op?currentPage=<%=pi.getCurrentPage() - 1%>'">&lt;</button>
			<%
				} else {
			%>
			...
			%>
			<button class="paging-b"
				onclick="location.href='<%=request.getContextPath()%>/list.op?currentPage=<%=i%>'"><%=i%></button>
			<%
				}
			%>
			...
			%>
			<button class="paging-b"
				onclick="location.href='<%=contextPath%>/list.op?currentPage=<%=pi.getCurrentPage() + 1%>'">&gt;</button>
			<%
				} else {
			%>
			<button class="paging-b" disabled>&gt;</button>
